...
test.describe('MapItems page', () => {
  // Clean up mappings and pantry entries from previous test runs
  test.beforeAll(async ({ browser }) => {
    const context = await browser.newContext()
    // Clean up Firestore data from previous test runs via admin SDK
    ...
    try {
      ...
      // App may already exist from a previous worker
  ...
  test('can select an item and see the modal picker', async ({ page, loginAs }) => {
    await loginAs('principiante')
    ...
    // The search input should be in the modal
    await expect(page.getByPlaceholder('Buscar ingrediente...')).toBeVisible()

    // Should show Omitir and Comida preparada buttons
    await expect(page.getByRole('button', { name: /omitir/i })).toBeVisible()
    await expect(page.getByRole('button', { name: /comida preparada/i })).toBeVisible()

    await page.screenshot({ path: `${RESULTS_DIR}/04-modal-picker.png`, fullPage: true })

    // Close modal with X button
    await page.getByLabel('Cerrar selector').click()
    await expect(page.locator('[data-testid="modal-backdrop"]')).not.toBeVisible()

    await page.screenshot({ path: `${RESULTS_DIR}/05-modal-closed.png`, fullPage: true })
  })

  test('can map an item via category accordion', async ({ page, loginAs }) => {
    const consoleErrors: string[] = []
    page.on('console', (msg) => {
      if (msg.type() === 'error') consoleErrors.push(msg.text())
    })
    ...
    // Categories should be visible (accordion headers)
    ...
    // Now pick the first ingredient in the expanded category (scope to modal)
    ...
    await page.screenshot({ path: `${RESULTS_DIR}/06-category-expanded.png`, fullPage: true })
    ...
    await page.screenshot({ path: `${RESULTS_DIR}/07-after-mapping.png`, fullPage: true })

    expect(parseInt(newPending ?? '0', 10)).toBe(initialPending - 1)
    expect(parseInt(newMapped ?? '0', 10)).toBe(1)
  })

  test('can map an item via search', async ({ page, loginAs }) => {
    ...
    await page.screenshot({ path: `${RESULTS_DIR}/07b-search-results.png`, fullPage: true })
  })
  ...
  test('can skip an item and see it in Omitidos', async ({ page, loginAs }) => {
    ...
    await page.screenshot({ path: `${RESULTS_DIR}/08-skipped-omitidos.png`, fullPage: true })
  })
  ...
  test('can restore a skipped item', async ({ page, loginAs }) => {
    ...
    await page.screenshot({ path: `${RESULTS_DIR}/09-after-restore.png`, fullPage: true })
  })

  test('can mark an item as comida preparada', async ({ page, loginAs }) => {
    await loginAs('comodo')
    ...
    // Wait for modal, then click "Comida preparada" button
    const preparedButton = page.getByRole('button', { name: /comida preparada/i })
    await expect(preparedButton).toBeVisible({ timeout: 10000 })

    await page.screenshot({ path: `${RESULTS_DIR}/10-prepared-button-visible.png`, fullPage: true })

    await preparedButton.click()

    // Wait for the async operation to complete and pending count to decrease
    await page.waitForFunction(
      (prev) => {
        const el = document.querySelector('.bg-yellow-50 .text-2xl')
        return el && parseInt(el.textContent ?? '0', 10) < prev
    ...
    const newPending = await pending.textContent()
    const newMapped = await mapped.textContent()
    console.log(`After marking prepared: Pending=${newPending}, Mapped=${newMapped}`)

    await page.screenshot({ path: `${RESULTS_DIR}/11-after-marking-prepared.png`, fullPage: true })
  ...
  test('shows items for all 4 test users', async ({ page, loginAs, logout }) => {
    ...
    for (const userKey of users) {
      ...
      await page.screenshot({
        path: `${RESULTS_DIR}/12-user-${userKey}.png`,
        fullPage: true,
      })
